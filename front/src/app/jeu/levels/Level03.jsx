...
export default function Level03({ nextLevelPath = '/jeu', homePath = '/jeu', difficulty = 'medium' }) {
  ...
  const [cameraConfig, setCameraConfig] = useState({ position: null, target: null })
  ...
  const bonusCombinations = [
    ...
    { pattern: ['scissors', 'rock', 'scissors'],  bonusType: 'SRS',       bonusPoints: 1 },
    { pattern: ['paper', 'rock', 'paper'],        bonusType: 'PRP',       bonusPoints: 1 },
  ]

  const gigaCombinations = [
    { pattern: ['well', 'well', 'well'], bonusType: 'GIGAWELL', bonusPoints: 2 },
  ]

  const resolveBonusCombo = (moves) =>
    bonusCombinations.find((c) => JSON.stringify(c.pattern) === JSON.stringify(moves)) ?? null

  const resolveGigaCombo = (moves) =>
    gigaCombinations.find((c) => JSON.stringify(c.pattern) === JSON.stringify(moves)) ?? null
  ...
  const triggerEndSequence = (winnerSide) => {
    ...
    setCameraConfig({ position: [0, 2, 20], target: [0, 2, 0] })
    setSceneChoices(
      winnerSide === 'left'
    ...
    endMessageTimeoutRef.current = setTimeout(() => {
      if (winnerSide === 'left') setShowVictoryMessage(true)
      else setShowLooserMessage(true)
    }, 900)
  }

  const handlePlayerFinished = (side, choice) => {
    if (!gameEnded || hasRedirected || choice !== 'winner' || side !== endWinnerSide) return
  ...
  const handleSkip = () => {
    if (!gameEnded || hasRedirected ) return
    setHasRedirected(true)
    setShowSkipButton(false)
    if (endMessageTimeoutRef.current) clearTimeout(endMessageTimeoutRef.current)
  ...
  const handleChoice = (playerChoice) => {
    ...
    let nextPlayerCombo = [...playerCombo, playerChoice]
    if (nextPlayerCombo.length > 3) {
      nextPlayerCombo.shift()
    }

    let nextBotCombo = [...botCombo, bot]
    if (nextBotCombo.length > 3) {
      nextBotCombo.shift()
    } 
    ...
    if (outcome === 'win') nextScore += 1
    if (outcome === 'lose') nextBotScore += 1 

    if (nextPlayerCombo.length === 3) {
      const gigaCombo  = resolveGigaCombo(nextPlayerCombo)
      const bonusCombo = !gigaCombo ? resolveBonusCombo(nextPlayerCombo) : null

      if (gigaCombo) {
        nextScore += gigaCombo.bonusPoints
        setBonusMessage(`🔥 GIGA BONUS ! +2 extra points for the ultimate combination !`)
        nextPlayerCombo = []
        if (bonusTimeoutRef.current) clearTimeout(bonusTimeoutRef.current)
        bonusTimeoutRef.current = setTimeout(() => setBonusMessage(''), 3000)
      } else if (bonusCombo) {
        nextScore += bonusCombo.bonusPoints
        
        if (bonusCombo.bonusType === 'SRS' || bonusCombo.bonusType === 'PRP') {
          setBonusMessage(`🎉 Bonus ! +1 extra point for a SECRET combination !`)
        } else {
          setBonusMessage(`🎉 Bonus ! +1 extra point for the SPECIAL combination !`)
        }
    ...
    if (nextBotCombo.length === 3) {
      const gigaCombo  = resolveGigaCombo(nextBotCombo)
      const bonusCombo = !gigaCombo ? resolveBonusCombo(nextBotCombo) : null

      if (gigaCombo) {
        nextBotScore += gigaCombo.bonusPoints
        setBonusMessage(`🤖 GIGA BONUS bot ! +2 extra points !`)
        nextBotCombo = []
        if (bonusTimeoutRef.current) clearTimeout(bonusTimeoutRef.current)
        bonusTimeoutRef.current = setTimeout(() => setBonusMessage(''), 3000)
      } else if (bonusCombo) {
        nextBotScore += bonusCombo.bonusPoints
        
        if (bonusCombo.bonusType === 'SRS' || bonusCombo.bonusType === 'PRP') {
          setBonusMessage(`🤖 The bot has a bonus ! +1 point for a SECRET combination !`)
        } else {
          setBonusMessage(`🤖 The bot has a bonus ! +1 point for the SPECIAL combination !`)
        }
    ...
    if (nextScore >= 10) {
      if (endGameTimeoutRef.current) clearTimeout(endGameTimeoutRef.current)
      endGameTimeoutRef.current = setTimeout(() => triggerEndSequence('left'), 10)
      return
    } else if (nextBotScore >= 10) {
      if (endGameTimeoutRef.current) clearTimeout(endGameTimeoutRef.current)
      endGameTimeoutRef.current = setTimeout(() => triggerEndSequence('right'), 10)
      return
    }
        ...
        <div className="flex flex-row justify-between items-start w-full gap-4">
          ...
          <div className="pointer-events-auto p-4 rounded-2xl bg-black/60 backdrop-blur-md border border-white/10 shadow-2xl text-right w-1/4 min-w-[140px]">
            ...
            <p className="text-[11px] sm:text-xs lg:text-sm text-stone-300 leading-relaxed">
              New hidden combos earn big bonuses! Master the sequence{' '}
              <span className="text-amber-400 font-bold uppercase">well ➔ well ➔ well</span> to trigger a giga bonus of{' '}
              <span className="font-bold text-amber-400">+2 points</span>!
            </p>
          </div>
